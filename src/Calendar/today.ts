...
export default function Today(bot: Telegraf<Context>) {
    async function todayCallback(ctx: Context) {
        ...
        return ctx.reply('События на сегодня', Markup.inlineKeyboard(meetings.map((meet, idx) => {
                return [Markup.button.callback(`${meet.name} 
                ${meet.all_day ? 'весь день' : `${meet.time_from} - ${meet.time_to}`}`,
    ...
    bot.action(/show_full/, ctx => {
            ...
            ctx.editMessageText(
                m.name, Markup.inlineKeyboard(
                    [Markup.button.callback('Назад', 'today_upd')]
                )
            )
        }
    )
    ...
    bot.action('today_upd', ctx => {
        return ctx.editMessageText('События на сегодня', Markup.inlineKeyboard(meetings.map((meet, idx) => {
                return [Markup.button.callback(`${meet.name} 
                ${meet.all_day ? 'весь день' : `${meet.time_from} - ${meet.time_to}`}`,
                    JSON.stringify({a: 'show_full', p: idx}))]
            })
